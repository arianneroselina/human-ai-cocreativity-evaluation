  ...
  if (summaries.length === 0) {
    return (
      <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-900">
        No feedback summaries found. Run <code className="font-mono">make process-data</code> with{" "}
      ...
      {summaries.map((item) => (
        ...
        >
          <div className="mb-3 flex items-start justify-between gap-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{item.workflowLabel}</h3>

              <p className="mt-1 font-mono text-xs text-gray-400">{item.workflow}</p>
            </div>
