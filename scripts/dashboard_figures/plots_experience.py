...
def plot_satisfaction_by_workflow(df):
    ...
    ax.set_title("Mean Satisfaction by Workflow")
    ...
    save_figure(
        ...
        "Mean Satisfaction by Workflow",
        "Participant-reported satisfaction after each round by workflow.",
    )
...
def plot_ai_experience_over_rounds(df):
    ...
    ax.set_title("AI Experience Ratings over Rounds")
    ax.set_xlabel("Round")
    ...
    save_figure(
        ...
        "AI Experience Ratings over Rounds",
        "Mean AI performance, collaboration, and creativity-support ratings over rounds for AI-supported workflows.",
    )


def plot_task_load_by_workflow(df):
    load_columns = {
    ...
    ax.set_title("Mean Task Load by Workflow")
    ax.set_xlabel("Workflow")
    ax.set_ylabel("Mean rating")
    ax.tick_params(axis="x", rotation=0)
    ax.legend(title="Task-load measure", bbox_to_anchor=(1.02, 1), loc="upper left")

    save_figure(
        fig,
        "23_task_load_by_workflow",
        "Mean Task Load by Workflow",
        "Mean perceived task-load ratings by workflow.",
    )
